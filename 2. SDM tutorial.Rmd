...
Bio 1 is average annual temp. So what does our map tell us about bio 1? Where is it warmest?
Bio 12 is annual precipitation.  Where is there the most rain?
```{r}
...
Now on your own, try zooming into bio 12, and getting a closer look at that map. write the code below
```{r}
...
make a new map, but pick a new color.
If you go to the link below, you can find a giant list of R colors in page 3.
https://www.nceas.ucsb.edu/~frazier/RSpatialGuides/colorPaletteCheatsheet.pdf
Make a map where the points are a new color.
```{r}
...
plot(maxent_results)
```
See the graph above. It says that Bio 7 and bio 12 were really important for predicting where the sloth would live.

#### Making our maps
There are a few key statistics tests.  When we get the maps, the left map is really key. Anywhere we see green, we have a really good chance of seeing sloths.
```{r}
